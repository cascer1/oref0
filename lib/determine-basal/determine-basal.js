...
function shortenLongTemps(currentTemp, deliverAt, basal, rT) {
 ...
 */
function determineEnableSMB(microBolusAllowed, profile, target_bg, meal_data) {
...
function determineSensitivityRatio(profile, target_bg, autosens_data) {
...
const determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime) {
    ...
    if (bg <= 10 || bg === 38 || noise >= 3 || minAgo > 12 || minAgo < -5 || (glucose_status.short_avgdelta === 0 && glucose_status.long_avgdelta === 0)) {
        return shortenLongTemps(currenttemp, deliverAt, basal, rT);
    }
    ...
    let sensitivityRatio = determineSensitivityRatio(profile, target_bg, autosens_data);
    ...
    // SMB is disabled by default, unless explicitly enabled in preferences.json
    let enableSMB = determineEnableSMB(microBolusAllowed, profile, target_bg, meal_data);
