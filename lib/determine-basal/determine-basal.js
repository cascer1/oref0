...
function calculate_expected_delta(target_bg, eventual_bg, bgi) {
    ...
    const five_min_blocks = (2 * 60) / 5;
    const target_delta = target_bg - eventual_bg;
...
/**
 * Shortens high temp basals and long-running zero temps to 30-minute zero temps
 * Does not modify temporary basals where 0 < rate < profile
 */
function shorten_long_temps(currentTemp, deliverAt, basal, rT) {
    if (currentTemp.rate >= basal) { // shorten high temp to 30 min zero temp
        rT.reason += ". Shortening " + currentTemp.duration + "m long temp to 30m zero temp. ";
        rT.deliverAt = deliverAt;
        rT.temp = 'absolute';
        rT.duration = 30;
        rT.rate = 0;
    } else if (currentTemp.rate === 0 && currentTemp.duration > 30) { // shorten long zero temp to 30 min
...
/**
 * Determine whether the current profile and circumstances allow SMB
 * SMB is disabled by default, unless explicitly enabled in preferences.json
 * @param microBolusAllowed {boolean}
 * @param profile {object}
 * @param target_bg {int}
 * @param meal_data {object}
 * @return {boolean} whether SMB is enabled in the current run
 */
function determine_enable_smb(microBolusAllowed, profile, target_bg, meal_data) {
...
/**
 * Determine sensitivity ratio based on temp target and autosens
 * @param profile pump profile
 * @param target_bg target BG in mg/dL
 * @param autosens_data autosens data
 * @returns {number} sensitivity ratio (e.g: 0.8, 1.04)
 */
function determine_sensitivity_ratio(profile, target_bg, autosens_data) {
    const highTempTargetRaisesSensitivity = profile.exercise_mode || profile.high_temptarget_raises_sensitivity;
    ...
    if (highTempTargetRaisesSensitivity && profile.temptargetSet && target_bg > normalTarget
...
/**
 * Calculates new target BG based on autosens data
 * Sensitivity increases target, resistance decreases target
 * @param target_bg normal target BG
 * @param autosens_data autosens data
 * @returns {number} new target BG
 */
function calculate_autosens_target_bg(target_bg, autosens_data) {
    let new_target_bg = round((target_bg - 60) / autosens_data.ratio) + 60;
    ...
    return new_target_bg;
}

/**
 * Applies sensitivity ratio to current basal rate
 * @param profile regular pump profile
 * @param sensitivityRatio {number} sensitivity ratio
 * @param profile_current_basal {number} current basal rate according to profile, rounded
 * @returns {number} adjusted basal rate
 */
function apply_sensitivity_to_basal(profile, sensitivityRatio, profile_current_basal) {
    let basal = profile.current_basal * sensitivityRatio;
...
const determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime) {
    let insulinScheduled;
    let insulinReq;
    let rate;
    let adjustedMaxBG;
    let adjustedTargetBG;
    let adjustedMinBG;
    let naive_eventualBG;
    let rT = {}; //short for requestedTemp
    ...
    if (bg <= 10 || bg === 38 || noise >= 3 || minAgo > 12 || minAgo < -5 || (glucose_status.short_avgdelta === 0 && glucose_status.long_avgdelta === 0)) {
        return shorten_long_temps(currenttemp, deliverAt, basal, rT);
    }
    ...
    let sensitivityRatio = determine_sensitivity_ratio(profile, target_bg, autosens_data);

    if (sensitivityRatio) {
        basal = apply_sensitivity_to_basal(profile, sensitivityRatio, profile_current_basal);
    }
    ...
    if (profile.temptargetSet) {
        ...
        if (profile.sensitivity_raises_target && autosens_data.ratio < 1 || profile.resistance_lowers_target && autosens_data.ratio > 1) {
            ...
            target_bg = calculate_autosens_target_bg(target_bg, autosens_data);
        }
    }
    ...
    const tempModulus = (lastTempAge + currenttemp.duration) % 30;
    ...
    if (currenttemp && iob_data.lastTemp && currenttemp.duration > 0) {
        ...
        const lastTempEnded = lastTempAge - iob_data.lastTemp.duration;
    ...
    let deviation = round(30 / 5 * (minDelta - bgi));
    ...
    if (glucose_status.noise >= 2) {
        ...
        const noisyCGMTargetMultiplier = Math.max(1.1, profile.noisyCGMTargetMultiplier);
        // don't allow maxRaw above 250
        const maxRaw = Math.min(250, profile.maxRaw);
    ...
    const expectedDelta = calculate_expected_delta(target_bg, eventualBG, bgi);
    ...
    const threshold = min_bg - 0.5 * (min_bg - 40);
    ...
    // SMB is disabled by default, unless explicitly enabled in preferences.json
    let enableSMB = determine_enable_smb(microBolusAllowed, profile, target_bg, meal_data);
    // enable UAM (if enabled in preferences)
    const enableUAM = (profile.enableUAM);
    ...
    const uci = round((minDelta - bgi), 1);
    ...
    let csf = sens / profile.carb_ratio;
    ...
    const maxCI = round(maxCarbAbsorptionRate * csf * 5 / 60, 1);
    if (ci > maxCI) {
        console.error("Limiting carb impact from", ci, "to", maxCI, "mg/dL/5m (", maxCarbAbsorptionRate, "g/h )");
        ci = maxCI;
    }
    let remainingCATimeMin = 3; // h; duration of expected not-yet-observed carb absorption
    ...
    const assumedCarbAbsorptionRate = 20; // g/h; maximum rate to assume carbs will absorb if no CI observed
    let remainingCATime = remainingCATimeMin;
    if (meal_data.carbs) {
        // if carbs * assumedCarbAbsorptionRate > remainingCATimeMin, raise it
        // so <= 90g is assumed to take 3h, and 120g=4h
        remainingCATimeMin = Math.max(remainingCATimeMin, meal_data.mealCOB / assumedCarbAbsorptionRate);
        const lastCarbAge = round((new Date(systemTime).getTime() - meal_data.lastCarbTime) / 60000);
        //console.error(meal_data.lastCarbTime, lastCarbAge);

        const fractionCOBAbsorbed = (meal_data.carbs - meal_data.mealCOB) / meal_data.carbs;
    ...
    const totalCI = Math.max(0, ci / 5 * 60 * remainingCATime / 2);
    // totalCI (mg/dL) / CSF (mg/dL/g) = total carbs absorbed (g)
    const totalCA = totalCI / csf;
    let remainingCarbsCap = 90; // default to 90
    let remainingCarbsFraction = 1;
    ...
    const remainingCarbsIgnore = 1 - remainingCarbsFraction;
    let remainingCarbs = Math.max(0, meal_data.mealCOB - totalCA - meal_data.carbs * remainingCarbsIgnore);
    ...
    const remainingCIpeak = remainingCarbs * csf * 5 / 60 / (remainingCATime / 2);
    //console.error(profile.min_5m_carbimpact,ci,totalCI,totalCA,remainingCarbs,remainingCI,remainingCATime);

    // calculate peak deviation in last hour, and slope from that to current deviation
    const slopeFromMaxDeviation = round(meal_data.slopeFromMaxDeviation, 2);
    // calculate lowest deviation in last hour, and slope from that to current deviation
    const slopeFromMinDeviation = round(meal_data.slopeFromMinDeviation, 2);
    // assume deviations will drop back down at least at 1/3 the rate they ramped up
    const slopeFromDeviations = Math.min(slopeFromMaxDeviation, -slopeFromMinDeviation / 3);
    ...
    const acid = Math.max(0, meal_data.mealCOB * csf / aci);
    ...
    let minIOBPredBG = 999;
    let minCOBPredBG = 999;
    let minUAMPredBG = 999;
    let minGuardBG = bg;
    let minCOBGuardBG = 999;
    let minUAMGuardBG = 999;
    let minIOBGuardBG = 999;
    let minZTGuardBG = 999;
    let minPredBG;
    let avgPredBG;
    let IOBpredBG = eventualBG;
    let maxIOBPredBG = bg;
    let maxCOBPredBG = bg;
    let maxUAMPredBG = bg;
    //var maxPredBG = bg;
    const eventualPredBG = bg;
    let lastIOBpredBG;
    let lastCOBpredBG;
    let lastUAMpredBG;
    let lastZTpredBG;
    let UAMduration = 0;
    let remainingCItotal = 0;
    const remainingCIs = [];
    const predCIs = [];
    try {
        iobArray.forEach(function (iobTick) {
            ...
            const predDev = ci * (1 - Math.min(1, IOBpredBGs.length / (60 / 5)));
            IOBpredBG = IOBpredBGs[IOBpredBGs.length - 1] + predBGI + predDev;
            // calculate predBGs with long zero temp without deviations
            const ZTpredBG = ZTpredBGs[ZTpredBGs.length - 1] + predZTBGI;
            // for COBpredBGs, predicted carb impact drops linearly from current carb impact down to zero
            // eventually accounting for all carbs (if they can be absorbed over DIA)
            const predCI = Math.max(0, Math.max(0, ci) * (1 - COBpredBGs.length / Math.max(cid * 2, 1)));
            const predACI = Math.max(0, Math.max(0, aci) * (1 - COBpredBGs.length / Math.max(acid * 2, 1)));
            ...
            const intervals = Math.min(COBpredBGs.length, (remainingCATime * 12) - COBpredBGs.length);
            const remainingCI = Math.max(0, intervals / (remainingCATime / 2 * 12) * remainingCIpeak);
            ...
            let COBpredBG = COBpredBGs[COBpredBGs.length - 1] + predBGI + Math.min(0, predDev) + predCI + remainingCI;
            const aCOBpredBG = aCOBpredBGs[aCOBpredBGs.length - 1] + predBGI + Math.min(0, predDev) + predACI;
            // for UAMpredBGs, predicted carb impact drops at slopeFromDeviations
            // calculate predicted CI from UAM based on slopeFromDeviations
            const predUCIslope = Math.max(0, uci + (UAMpredBGs.length * slopeFromDeviations));
            // if slopeFromDeviations is too flat, predicted deviation impact drops linearly from
            // current deviation down to zero over 3h (data points every 5m)
            const predUCImax = Math.max(0, uci * (1 - UAMpredBGs.length / Math.max(3 * 60 / 5, 1)));
            //console.error(predUCIslope, predUCImax);
            // predicted CI from UAM is the lesser of CI based on deviationSlope or DIA
            const predUCI = Math.min(predUCIslope, predUCImax);
            if (predUCI > 0) {
                //console.error(UAMpredBGs.length,slopeFromDeviations, predUCI);
                UAMduration = round((UAMpredBGs.length + 1) * 5 / 60, 1);
            }
            let UAMpredBG = UAMpredBGs[UAMpredBGs.length - 1] + predBGI + Math.min(0, predDev) + predUCI;
            ...
            let insulinPeakTime = 60;
            // add 30m to allow for insluin delivery (SMBs or temps)
            insulinPeakTime = 90;
            const insulinPeak5m = (insulinPeakTime / 60) * 12;
    ...
    const fractionCarbsLeft = meal_data.mealCOB / meal_data.carbs;
    ...
    let minZTUAMPredBG = minUAMPredBG;
    ...
    if (minZTGuardBG < threshold) {
        ...
        const blendPct = (minZTGuardBG - threshold) / (target_bg - threshold);
        const blendedMinZTGuardBG = minUAMPredBG * blendPct + minZTGuardBG * (1 - blendPct);
    ...
    if (meal_data.carbs) {
        ...
        if (!enableUAM && minCOBPredBG < 999) {
            ...
            const blendedMinPredBG = fractionCarbsLeft * minCOBPredBG + (1 - fractionCarbsLeft) * minZTUAMPredBG;
    ...
    let carbsReqBG = naive_eventualBG;
    if (carbsReqBG < 40) {
    ...
    let minutesAboveMinBG = 240;
    let minutesAboveThreshold = 240;
    if (meal_data.mealCOB > 0 && (ci > 0 || remainingCIpeak > 0)) {
    ...
    const zeroTempDuration = minutesAboveThreshold;
    // BG undershoot, minus effect of zero temps until hitting min_bg, converted to grams, minus COB
    let zeroTempEffect = profile.current_basal * sens * zeroTempDuration / 60;
    // don't count the last 25% of COB against carbsReq
    const COBforCarbsReq = Math.max(0, meal_data.mealCOB - 0.25 * meal_data.carbs);
    let carbsReq = (bgUndershoot - zeroTempEffect) / csf - COBforCarbsReq;
    ...
    if (eventualBG < min_bg) { // if eventual BG is below target:
        ...
        let naiveInsulinReq = Math.min(0, (naive_eventualBG - target_bg) / sens);
        naiveInsulinReq = round(naiveInsulinReq, 2);
        if (minDelta < 0 && minDelta > expectedDelta) {
            // if we're barely falling, newinsulinReq should be barely negative
            const newinsulinReq = round((insulinReq * (minDelta / expectedDelta)), 2);
    ...
    if (iob_data.iob > max_iob) {
        ...
        const lastBolusAge = round((new Date(systemTime).getTime() - iob_data.lastBolusTime) / 60000, 1);
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            const microBolus = Math.floor(Math.min(insulinReq / 2, maxBolus) * roundSMBTo) / roundSMBTo;
            // calculate a long enough zero temp to eventually correct back up to target
            worstCaseInsulinReq = (target_bg - (naive_eventualBG + minIOBPredBG) / 2) / sens;
            durationReq = round(60 * worstCaseInsulinReq / profile.current_basal);
            ...
            const nextBolusMins = round(SMBInterval - lastBolusAge, 0);
            const nextBolusSeconds = round((SMBInterval - lastBolusAge) * 60, 0) % 60;
